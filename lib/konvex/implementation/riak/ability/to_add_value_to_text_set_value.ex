defmodule Konvex.Implementation.Riak.Ability.ToAddValueToTextSetValue do
  defmacro __using__(
    ...
    quote do
      ...
      def add_value_to_text_set_value("" = _empty_key, value) when is_binary(value) do
        # :riakc does not support empty keys, so no way to store such
        :key_not_found
      end

      def add_value_to_text_set_value(<<_, _ :: binary>> = key, value) when is_binary(value) do
        using unquote(quoted_riak_connection), fn connection_pid ->
          case :riakc_pb_socket.fetch_type(
               ...
               ) do
            {
              ...
              if (fetched_set_values |> Enum.member?(value)) do
